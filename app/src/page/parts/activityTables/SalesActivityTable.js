...
export default function SalesActivityTable({ collectionAddress }) {
			...
			loadedItems.forEach(item => {
				...
				if (item.price) {
					tmp.push({
						...
						items: item.quantity,
						price: parseFloat(item.price[0].price / 1e18).toFixed(4),
						priceChange: 23,
	...
	return (
		<>
			<div className="table-row header-row pt-2 c7-2fr">
				<span>Item</span>
				<span>Price</span>
				<span>Items</span>
				<span>From</span>
			...
			{rows.map((row, idx) => {
				if (idx < index * limit) {
					return (
						<div key={idx} className="table-row c7-2fr non-clickable">
							<span>
							...
							<span><NumWithChange icon={quoteCurrency} num={row.price} /></span>
